...
class Tokenizer {
  ...
  tokenize(rawText) {
    ...
    for (let line = 0; line < text.length; line++) {
      ignoreLine: for (let column = 0; column < text[line].length; column++) {
        if (isString) {
          ...
          switch (text[line][column]) {
            case '"':
              this.newToken('val', text[line][column], [
                line + 1,
                column + 1,
                line + 1,
                column + 2
            ...
            case ')':
              this.pushToken()
              this.newToken(text[line][column], text[line][column], [
                line + 1,
                column + 1,
                line + 1,
                column + 2
...
class Parser {
  tokenizer: Tokenizer
  tokens: IterableIterator<[number, any]>
  constructor() {
    ...
    while (((token = this.tokens.next()), !token.done)) {
      ...
      switch (token.type) {
        ...
        case '(': {
          ...
          stack.push(
            new Token('array', foamArray, [
              token.range[0],
              token.range[1],
              closingToken.range[2],
              closingToken.range[3]
            ])
          )
          break
        }
    ...
    while (((token = this.tokens.next()), !token.done)) {
      ...
      switch (token.type) {
        ...
        case '(': {
          const foamArray = this.parseArr()
          const closingToken = foamArray.pop()
          obj[stack.map((o) => o.value).join(' ')] = new Token(
            'array',
            foamArray,
            [
              token.range[0],
